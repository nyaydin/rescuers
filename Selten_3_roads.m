...
strategies_vec_t1 = propens_vec_0(1:3);
probability_strategies_vec_t1 = strategies_vec_t1/sum(strategies_vec_t1);
position_strategy_vec_t1 = position_strategy_vec(1:3);
...
for t = 1 : T 
    if t == 1
        ...
        % Nr. players choosing strategy 3 (Tertiary)
        n_T_t1 = sum(strategy_t1 == 3);
        ...
        propensity_vector_before_draw_t1(strategy_t1 == 3) = strategies_vec_t1(3);
    ...
    % For the remaining time periods          
        ...
        for player = 1 : N
            ...
            elseif strategy_tp1 == 4
                ...
                if payoff_t < median_all_prev_payoffs
                    ...
                    elseif route_choice_t == 3
                        route_choice(player,t) = 1;
                    else
                        route_choice(player,t)= 3
                    end
        ...
        % Nr. players choosing strategy 3 (Tertiary)
        n_T = sum(route_choice(:,t) == 3);
        % Penalty for players choosing strategy 1 (Main)
        tM = Pen_M + Multi_M*n_M;
        % Penalty for players choosing strategy 2 (Secondary)
        tS = Pen_S + Multi_S*n_S;
        % Penalty for players choosing strategy 3 (Tertiary)
        tT = Pen_T + Multi_T*n_T;
        % Payoff for players choosing strategy 1 (Main)
        a_M = max_payoff - tM;
        % Payoff for players choosing strategy 2 (Secondary)
        a_S = max_payoff - tS;
        % Payoff for players choosing strategy 3 (Tertiary)
        a_T = max_payoff - tT;       
                
        % Payoff vector of the 18 players
        payoff_matrix(route_choice(:,t) == 1,t) = a_M;
        payoff_matrix(route_choice(:,t) == 2,t) = a_S;
        payoff_matrix(route_choice(:,t) == 3,t) = a_T;
        % Update propensity
        for player = 1 : N
...
nr_players_tertiary = sum(route_choice == 3);
max_nr_players_tertiary = max(nr_players_tertiary);
vec_max_nr_players_tertiary = max_nr_players_tertiary*ones(1,T);
min_nr_players_tertiary = min(nr_players_tertiary);
vec_min_nr_players_tertiary = min_nr_players_tertiary*ones(1,T);
ave_nr_players_tertiary = mean(nr_players_tertiary);
vec_ave_nr_players_tertiary = ave_nr_players_tertiary*ones(1,T);
...
subplot(3,1,3)
plot(time_vec,nr_players_tertiary)
title('Number of players choosing the secondary road')
hold on
plot(time_vec,[vec_max_nr_players_tertiary;...
               vec_min_nr_players_tertiary;...
               vec_ave_nr_players_tertiary])
ylim(ylim_plot)
xlim(xlim_plot)
xlabel('Round')
legend('Nr. players','Max nr. players','Min nr. players','Average nr. players')
